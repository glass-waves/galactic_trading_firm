#!/usr/bin/env bash
# run backtest on every trading day in 2024 (mature bull + election year volatility)
# generates ~251 trading dates dynamically, skipping weekends and NYSE holidays
# logs all verbose output to logs/backtest_2024_full.log for post-analysis
#
# market context: continued AI bull, rate cut speculation, election year vol spikes,
# aug 5 VIX spike to 65 (japan carry trade unwind), NVDA 10:1 split jun 10.
# tests robustness in news-driven choppy conditions with regime switching.
#
# note: NVDA split 10:1 on 2024-06-10. alpaca provides split-adjusted data,
# so prices should be consistent across the year.
#
# usage: ./scripts/backtest_2024.sh [--compound] [extra backtest args...]
# examples:
#   ./scripts/backtest_2024.sh
#   ./scripts/backtest_2024.sh --compound
#   ./scripts/backtest_2024.sh --ticker-override "AAPL:entry_threshold=0.35"
...
if (( $(echo "$max_drawdown > 0" | bc -l) )); then
    dd_pct=$(echo "scale=1; $max_drawdown * 100 / $CAPITAL" | bc -l)
