#!/usr/bin/env bash
# run backtest on a fixed set of 20 dates and report aggregate results with rubric
# usage: ./scripts/backtest_20days.sh
...
# 1. total P&L score (primary)
if (( $(echo "$total_pnl >= 2000" | bc -l) )); then pnl_score=5
elif (( $(echo "$total_pnl >= 500" | bc -l) )); then pnl_score=4
elif (( $(echo "$total_pnl >= 0" | bc -l) )); then pnl_score=3
elif (( $(echo "$total_pnl >= -2000" | bc -l) )); then pnl_score=2
else pnl_score=1; fi
score_total=$(( score_total + pnl_score * 3 ))  # weight 3x
metric_count=$(( metric_count + 3 ))
printf "  total P&L:          %d/5  (>2k=5, >500=4, >0=3, >-2k=2, else=1)\n" "$pnl_score"
...
# 4. trade frequency (want 2-15 avg trades/day)
if [[ "$day_count" -gt 0 ]]; then
...
# 5. tail risk (biggest loss as % of capital)
if [[ -n "$biggest_loss_date" ]]; then
...
# 6. win/loss asymmetry (avg win / avg loss)
if [[ "$win_days" -gt 0 ]] && [[ "$loss_days" -gt 0 ]]; then
